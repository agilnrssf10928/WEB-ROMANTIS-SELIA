...
async function readPhotos() {
  ...
  const blob = result.blobs
    .filter((item) => item.pathname.startsWith(DATA_PATH))
...
module.exports = async function handler(request, response) {
  ...
  if (!process.env.BLOB_READ_WRITE_TOKEN) {
    ...
    return response.status(503).json({ error: 'Vercel Blob belum tersambung. Tambahkan BLOB_READ_WRITE_TOKEN di Project Settings > Environment Variables.' });
  }
